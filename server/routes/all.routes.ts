...
const searchController = createSearchController({ ok, error });

const respondForbiddenFromAuthError = (res: Response, err: unknown) => {
  const status = (err as Error & { status?: number }).status || 403;
  return res
    .status(status)
    .json(error(status === 401 ? "UNAUTHORIZED" : "FORBIDDEN", "Forbidden"));
};
...
export async function registerAllRoutes(app: Express): Promise<void> {
  ...
  app.delete("/api/admin/disputes/:id/evidence", async (req, res) => {
    ...
    // Fetch dispute
    const [dispute] = await db
      .select({ id: reviewDisputes.id, evidenceKey: reviewDisputes.evidenceKey })
      .from(reviewDisputes)
      .where(eq(reviewDisputes.id, disputeId));
