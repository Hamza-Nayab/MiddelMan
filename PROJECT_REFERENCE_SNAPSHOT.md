...
- **`server/routes.ts`** - All API endpoints (auth, profiles, links, reviews, admin, analytics, disputes, avatar/evidence uploads)
- **`shared/schema.ts`** - Database schema: users, profiles, links, reviews, profile_daily_stats, admin_audit_logs, review_disputes
- **`client/src/lib/api.ts`** - Centralized API client with all endpoints including disputes and admin analytics
- **`client/src/pages/dashboard.tsx`** - Seller dashboard, analytics charts, profile editor, reviews with dispute creation
- **`client/src/pages/my-reviews.tsx`** - Seller reviews management with dispute functionality
...
- **`client/src/pages/admin-users.tsx`** - Admin user management (disable/enable, role changes)
- **`client/src/pages/admin-reviews.tsx`** - Admin review moderation
- **`client/src/pages/admin-disputes.tsx`** - Admin dispute resolution with evidence viewer
- **`client/src/pages/admin-analytics.tsx`** - Platform analytics dashboard
...
- **`server/r2.ts`** - Cloudflare R2 integration for avatar and dispute evidence uploads
...
| POST   | /api/me/reviews/:reviewId/dispute          | Yes     | ✅     | Create dispute for review               |
| POST   | /api/me/reviews/:reviewId/dispute/evidence | Yes     | ✅     | Upload dispute evidence (PDF/images)    |
...
| GET    | /api/admin/disputes                        | Yes\*   | ✅     | List disputes with filters/pagination   |
| PATCH  | /api/admin/disputes/:id/resolve            | Yes\*   | ✅     | Resolve dispute (valid/rejected)        |
| DELETE | /api/admin/disputes/:id/evidence           | Yes\*   | ✅     | Delete dispute evidence from R2         |
| GET    | /api/admin/sellers/:sellerId               | Yes\*   | ✅     | Get seller detail (admin view)          |
| GET    | /api/admin/analytics/overview              | Yes\*   | ✅     | Platform analytics overview (7/30 days) |
...
- `targetDisputeId` (integer, nullable) - Dispute affected by action
- `details` (jsonb, nullable) - Additional context (outcome, resolution notes, etc.)
...
- `RESOLVE_DISPUTE` - Dispute resolved by admin
- `DELETE_DISPUTE_EVIDENCE` - Evidence file deleted from R2
- `demote_admin` - Admin demoted to regular user
...
**Purpose:** Allow sellers to dispute unfair reviews with evidence upload support.

**Columns:**

- `id` (serial, PRIMARY KEY) - Dispute ID
- `reviewId` (integer, NOT NULL, UNIQUE, FK→reviews.id, onDelete=cascade) - Disputed review (one dispute per review)
- `sellerId` (integer, NOT NULL, FK→users.id) - Seller filing dispute
- `status` (enum: disputeStatusEnum, default='open') - Dispute status
- `reason` (varchar, NOT NULL) - Why seller is disputing (predefined or custom)
- `message` (text, nullable) - Additional context from seller
- `evidenceUrl` (text, nullable) - Public URL to uploaded evidence (R2)
- `evidenceMime` (text, nullable) - MIME type of evidence (image/\* or application/pdf)
- `evidenceKey` (text, nullable) - R2 object key for deletion
...
- `resolutionNote` (text, nullable) - Admin's resolution explanation
- `deletedEvidenceAt` (timestamp with timezone, nullable) - When evidence was deleted by admin

**Indexes:**

- PRIMARY KEY on `id`
- UNIQUE on `reviewId` (one dispute per review)
- `review_disputes_status_created_idx` on (`status`, `createdAt`)

**Status Types:**

- `open` - Awaiting admin review
- `resolved_valid` - Dispute approved (review hidden)
- `resolved_rejected` - Dispute rejected (review remains visible)

**Evidence Storage:**

- Stored in Cloudflare R2 bucket
- Path: `review-disputes/{disputeId}/{timestamp}.{ext}`
- Supported: Images (JPEG, PNG, WebP) and PDFs
- Max size: 10MB
- Server-side validation with Sharp (images) and signature check (PDFs)

**Queries:**

- `INSERT INTO review_disputes` - seller files dispute via `/api/me/reviews/:reviewId/dispute`
- `SELECT FROM review_disputes WHERE status = 'open'` - admin queue
- `UPDATE review_disputes SET status = ..., resolvedByAdminId = ..., resolvedAt = ..., resolutionNote = ...` - admin resolution
- `SELECT FROM review_disputes WHERE reviewId = ...` - check if dispute exists (before creating)
- `UPDATE review_disputes SET evidenceUrl = ..., evidenceKey = ..., evidenceMime = ...` - after evidence upload
...
CREATE TYPE admin_action AS ENUM ('DISABLE_USER', 'ENABLE_USER', 'PROMOTE_ADMIN', 'DEMOTE_ADMIN', 'HIDE_REVIEW', 'UNHIDE_REVIEW', 'RESOLVE_DISPUTE', 'DELETE_DISPUTE_EVIDENCE');
CREATE TYPE dispute_status AS ENUM ('open', 'resolved_valid', 'resolved_rejected');
```
...
| `createReviewDispute(reviewId, reason)`               | POST   | /api/me/reviews/:reviewId/dispute    | `{ dispute: ReviewDispute }` |
| `uploadDisputeEvidence(disputeId, file)`              | POST   | /api/me/disputes/:disputeId/evidence | `{ dispute: ReviewDispute }` |
| `adminGetUsers()`                                     | GET    | /api/admin/users                     | `{ users: User[] }`          |
| `adminSetUserRole(id, role)`                          | PATCH  | /api/admin/users/:id/role            | `{ user: User }`             |
| `adminDisableUser(id, disabled)`                      | PATCH  | /api/admin/users/:id/disable         | `{ user: User }`             |
...
| `adminGetDisputes(filters)`                           | GET    | /api/admin/disputes                  | `DisputesResponse`           |
| `adminResolveDispute(id, outcome, hideReview, notes)` | PATCH  | /api/admin/disputes/:id/resolve      | `{ dispute: ReviewDispute }` |
| `adminDeleteDisputeEvidence(id)`                      | DELETE | /api/admin/disputes/:id/evidence     | `{ dispute: ReviewDispute }` |
| `adminGetSellerDetail(userId)`                        | GET    | /api/admin/sellers/:userId           | `SellerDetailResponse`       |
| `adminGetAnalyticsOverview(days)`                     | GET    | /api/admin/analytics/overview        | `AnalyticsOverview`          |
...
3. **Reviews Tab:**
   ...
   - **NEW:** Dispute unfair reviews with reason selection
   - **NEW:** Upload evidence (images/PDFs) for disputes
   - View dispute status and admin resolution notes

4. **Analytics Tab:**
   - Real-time analytics from `profile_daily_stats` table
   - 7-14 day view counts and click tracking
...
- **NEW:** Create disputes for reviews received (seller-only feature, from dashboard)

---

## Admin Pages

### Admin Overview (`client/src/pages/admin.tsx`)

**Route:** `/admin` (admin only)  
**Purpose:** Admin dashboard with navigation tabs

**Features:**

- Tab navigation to all admin sections: Users, Reviews, Disputes, Analytics
- Quick stats overview cards
- Tabbed interface for different management areas

### Admin Users Page (`client/src/pages/admin-users.tsx`)
...
- `useQuery("admin-users")` → `api.adminGetUsers()`

**Features:**

- User list with search/filter (role, disabled status)
- Set user roles (buyer, seller, admin)
- Enable/disable user accounts (disables login + profile access)
- View user details and activity
- Bootstrap master admin functionality

### Admin Reviews Page (`client/src/pages/admin-reviews.tsx`)
...
- Review list with filters (visibility, rating)
- Hide/show reviews (removes from public view)
- View full review details and seller context
- Audit trail via admin_audit_logs

### Admin Disputes Page (`client/src/pages/admin-disputes.tsx`)

**Route:** `/admin/disputes` (admin only)  
**Purpose:** Resolve seller disputes against unfair reviews

**Data Loading:**

- `useQuery("admin-disputes", filters)` → `api.adminGetDisputes({ status?, sellerId?, page?, limit? })`

**Features:**

- Dispute list with pagination and filters (status: open/resolved_valid/resolved_rejected)
- View dispute reason, evidence (images/PDFs), and associated review
- Evidence preview with download and delete options
- Resolve disputes with outcomes:
  - **Valid:** Review was unfair, optionally hide the review
  - **Rejected:** Dispute not justified, review remains visible
- Delete dispute evidence files from R2 storage
- Real-time status updates

### Admin Analytics Page (`client/src/pages/admin-analytics.tsx`)
...
- `useQuery("admin-analytics-overview", dateRange)` → `api.adminGetAnalyticsOverview({ dateRange: "7d" | "30d" })`

**Features:**

- Platform metrics: total users, sellers, buyers, reviews
- Activity metrics: active users (last 7/30 days), new signups, profile views, review submissions
- Engagement stats: ratings distribution, verified seller count
- Date range selector (7 days vs 30 days)
- Real-time data fetched from profile_daily_stats aggregations
- Server-side caching (60s TTL) for performance

### Admin User Detail Page (`client/src/pages/admin-seller-detail.tsx`)

**Route:** `/admin/sellers/:userId` (admin only)  
**Purpose:** Comprehensive view of any user (buyer, seller, or admin)

**Data Loading:**

- `useQuery("admin-seller-detail", userId)` → `api.adminGetSellerDetail(userId)`

**Features:**

- Complete user profile: display name, bio, avatar, social links
- Account stats: total reviews received, rating distribution
- Account management: enable/disable, view role, master admin badge
- Links section: all user links with sort order
- Reviews section (tabbed):
  - **Received Tab:** Latest 20 reviews this user received as a seller
  - **Given Tab:** Latest 20 reviews this user gave as a buyer
  - Shows hidden reviews, ratings, author info, dates
- Disputes section: list of disputes created by this user (if seller) with status
...
export type ReviewDispute = {
  ...
  deletedEvidenceAt: string | null;
  createdAt: string;
  updatedAt: string;
};

export type DisputeWithDetails = ReviewDispute & {
  review: Review;
  seller: {
    userId: number;
    username: string | null;
...
- **Admin Panel Features (✅ Implemented):**
  - ✅ Admin dashboard UI with tab navigation
  - ✅ User list and management (roles, disable/enable accounts)
  - ✅ Review moderation (hide/show reviews)
  - ✅ Dispute resolution system (view, resolve, delete evidence)
  - ✅ Platform analytics dashboard (7/30 day views, top sellers)
  - ✅ Seller detail view (comprehensive seller information)
  - ⏳ Verification request approval UI (pending)
